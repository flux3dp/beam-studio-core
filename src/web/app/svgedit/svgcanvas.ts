...
export default $.SvgCanvas = function (container, config) {
  // Alias Namespace constants
  var NS = svgedit.NS;

  // Default configuration options
  var curConfig: { [key: string]: any } = {
  ...
  var opac_ani: any = document.createElementNS(NS.SVG, 'animate');
  $(opac_ani).attr({
  ...
  let pathActions = canvas.pathActions = (function () {
    ...
    return {
          ...
          if (hasMoved) {
            ...
            if (!evt.shiftKey) {
              ...
              if (id.substr(0, 14) === 'pathpointgrip_') {
                ...
                let pointIndex = svgedit.path.path.selectedPointIndex = parseInt(id.substr(14));
                svgedit.path.path.clearSelection();
                svgedit.path.path.addPtsToSelection(pointIndex);
              }
            }
      ...
      deletePathNode: function () {
        ...
        if ((window as any).opera) { // Opera repaints incorrectly
  ...
  let removeDefaultLayerIfEmpty = this.removeDefaultLayerIfEmpty = () => {
  ...
  this.importSvgString = async function (xmlString, _type, layerName) {
    ...
    function parseSvg(svg: Element, type) {
      ...
      function _parseSvgByLayer() {
        const defNodes = Array.from(svg.childNodes).filter((node: Element) => 'defs' === node.tagName);
        let defChildren = [];
        defNodes.map((def: Element) => {
        ...
        const isValidLayeredSvg = layerNodes.every((node: Element) => {
      ...
      function _parseSvgByColor(svg) {
        ...
        function getAllColorInNodes(nodes) {
          ...
          function traverseToGetAllColor(frontierNode) {
            Array.from(frontierNode.childNodes).map((child: Element) => {
        ...
        function filterColor(filter, node) {
          ...
          children.map((grandchild: Element) => {
        ...
        originLayerNodes.map((child: Element) => {
      ...
      function _parseSvgByNolayer(svg) {
        ...
        const defNodes = Array.from(svg.childNodes).filter((node: Element) => 'defs' === node.tagName);
        const styleNodes = Array.from(svg.childNodes).filter((node: Element) => 'style' === node.tagName);
        let defChildren = [];
        defNodes.map((def: Element) => {
          defChildren = defChildren.concat(Array.from(def.childNodes));
        });
    ...
    function rgbToHex(rgbStr) {
      ...
      let hex = (Math.round(parseFloat(rgb[0]) * 2.55) * 65536 + Math.round(parseFloat(rgb[1]) * 2.55) * 256 + Math.round(parseFloat(rgb[2]) * 2.55)).toString(16);
      if (hex === 'NaN') {
        hex = '0';
      }
      while (hex.length < 6) {
  ...
  this.updateLayerColorFilter = (layer) => {
    ...
    let filter = Array.from(layer.childNodes).filter((child: Element) => child.tagName === 'filter')[0] as Element;
    if (filter) {
  ...
  this.booleanOperationSelectedElements = function (mode, isSubCmd = false) {
    ...
    for (let i = len - 1; i >= 0; --i) {
      let clipper = new (window as any).ClipperLib.Clipper();
      ...
      const paths = (window as any).ClipperLib.dPathtoPointPathsAndScale(dpath, rotation, scale);
      if (i === len - 1) {
  ...
  */
  this.imageToSVG = async function (img) {
    ...
    let svgStr: string = await new Promise((resolve) => {
  ...
  this.flipSelectedElements = async function (horizon = 1, vertical = 1) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      let cmd;
      let stack: { elem?: Element, originalAngle?: number }[] = [{ elem }];
      while (stack.length > 0) {
        let { elem: topElem, originalAngle } = stack.pop();
