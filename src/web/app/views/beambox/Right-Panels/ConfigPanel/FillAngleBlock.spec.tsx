...
const mockContextState = { fillAngle: { value: 87, hasMultiValue: false } };
...
describe('test FillAngleBlock', () => {
  ...
  it('should render correctly when type is panel-item', () => {
    const { container } = render(
      ...
      >
        <FillAngleBlock type="panel-item" />
      </ConfigPanelContext.Provider>
    );
    expect(container).toMatchSnapshot();
  });
  ...
  test('onChange should work', () => {
    ...
    expect(mockDispatch).toHaveBeenLastCalledWith({
      type: 'change',
      payload: { fillAngle: 88, configName: 'CUSTOM_PRESET_CONSTANT' },
    });
    expect(mockBatchCommand).toBeCalledTimes(1);
    expect(mockBatchCommand).lastCalledWith('Change fillAngle');
    expect(batchCmd.count).toBe(1);
    expect(mockWriteData).toBeCalledTimes(4);
    expect(mockWriteData).toHaveBeenNthCalledWith(1, 'layer1', 'fillAngle', 88, { batchCmd });
    ...
    expect(mockWriteData).toHaveBeenNthCalledWith(3, 'layer2', 'fillAngle', 88, { batchCmd });
    expect(mockWriteData).toHaveBeenNthCalledWith(
      4,
      'layer2',
      'configName',
