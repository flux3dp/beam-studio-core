...
describe('test BasePreviewManager', () => {
  ...
  test('getPhotoAfterMoveTo', async () => {
    ...
    const mockWaitUntilEstimatedMovementTime = jest.spyOn(basePreviewManager, 'waitUntilEstimatedMovementTime');
    mockSelect.mockResolvedValue({ success: true });
    ...
    expect(mockWaitUntilEstimatedMovementTime).toBeCalledTimes(1);
