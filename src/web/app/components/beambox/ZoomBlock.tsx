...
const ZoomBlock = ({ getZoom, setZoom, resetView, isPathPreviewing }: Props): JSX.Element => {
  ...
  const [isRatioInited, setIsRatioInited] = useState(false);
    ...
    if (!isRatioInited) {
      const getInitRatio = () => {
        ...
        if (ratio === null) {
          ...
          setIsRatioInited(true);
          setDisplayRatio(ratio);
        }
      };
      getInitRatio();
    }
  }, [isRatioInited, calculateCurrentRatio]);
  ...
  const setRatio = useCallback((ratioPercentage: number) => {
    const newRatio = ratioPercentage / 100;
    const targetZoom = newRatio * dpmm;
    setZoom(targetZoom);
  }, [dpmm, setZoom]);

  const zoomIn = useCallback((currentRatio: number) => {
  ...
  const zoomOut = useCallback((currentRatio: number) => {
    const ratioInPercent = Math.round(currentRatio * 100);
    let targetRatio: number;
    if (ratioInPercent <= 500) {
  ...
  return (
    <div
      className={classNames(
        styles.container, { [styles['path-preview']]: isPathPreviewing, [styles.mobile]: isMobile }
      )}
    >
      ...
      <ContextMenu id="zoom-block-contextmenu">
        <MenuItem attributes={{ id: 'fit_to_window' }} onClick={resetView}>{lang.fit_to_window}</MenuItem>
        <MenuItem attributes={{ id: 'ratio_25%' }} onClick={() => setRatio(25)}>25 %</MenuItem>
        <MenuItem attributes={{ id: 'ratio_50%' }} onClick={() => setRatio(50)}>50 %</MenuItem>
        <MenuItem attributes={{ id: 'ratio_75%' }} onClick={() => setRatio(75)}>75 %</MenuItem>
        <MenuItem attributes={{ id: 'ratio_100%' }} onClick={() => setRatio(100)}>100 %</MenuItem>
        <MenuItem attributes={{ id: 'ratio_150%' }} onClick={() => setRatio(150)}>150 %</MenuItem>
        <MenuItem attributes={{ id: 'ratio_200%' }} onClick={() => setRatio(200)}>200 %</MenuItem>
      </ContextMenu>
    </div>
  );
};
