...
export default {
  ...
  findFonts,
  getAvailableFonts,
  async substituteFont(postscriptName: string, text: string) {
    // TODO
    return localFontHelper.substituteFont(postscriptName, text);
    const font = (await getAvailableFonts()).filter((f) => f.postscriptName === postscriptName);
