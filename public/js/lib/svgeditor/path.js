...
svgedit.path.Path.prototype.init = function() {
	...
	for (i = 0; i < len; i++) {
		...
		if (seg.type === 2) {
			...
			const nodePoint = new svgedit.path.PathNodePoint(seg.item.x, seg.item.y, seg, this);
...
svgedit.path.Path.prototype.addSeg = function(index, interpolation = 0.5) {
  let t = interpolation;
	// Adds a new segment
	...
	case 6: //make it a curved segment to preserve the shape (WRS)
		...
		var p0_x = prev.item.x * t + seg.item.x1 * (1-t);
		var p1_x = seg.item.x1 * t + seg.item.x2 * (1-t);
		var p2_x = seg.item.x2 * t + seg.item.x * (1-t);
		var p01_x = p0_x * t + p1_x * (1-t);
		var p12_x = p1_x * t + p2_x * (1-t);
		new_x = p01_x * t + p12_x * (1-t);
		var p0_y = prev.item.y * t + seg.item.y1 * (1-t);
		var p1_y = seg.item.y1 * t + seg.item.y2 * (1-t);
		var p2_y = seg.item.y2 * t + seg.item.y * (1-t);
		var p01_y = p0_y * t + p1_y * (1-t);
		var p12_y = p1_y * t + p2_y * (1-t);
		new_y = p01_y * t + p12_y * (1-t);
